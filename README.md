...
它不是把几个人设拼在一起轮流说话，也不是那种看着热闹、实则没有秩序的 multi-agent demo。
真正的骨架是：
...
更早的起点其实很简单：我是个想法很多的人。同一件事，脑子里经常会同时跑出几种互相冲突的判断。但这些判断，大多数时候没有地方真正展开。不是因为没人能听懂，而是现实里很少有人既愿意听，又能给出足够有力的回应。

时间久了，这些想法只能在脑子里自己打转。

后来我开始想：能不能做一个东西，让那些我真正敬佩的人来评价我的观点？

这个项目最早的名字其实叫 **"擂台"**。当时的直觉是：既然想法需要被检验，那就把它们放上擂台。

但很快我发现，生活里不是所有问题都适合"打"。有些时候你不是想被击败，也不是想赢，而是根本不知道该怎么办，只想先有人帮你看看问题到底出在哪。
...
医生侧 skill 主要建立在 [nuwa-skill](https://github.com/alchaincyf/nuwa-skill) 这套蒸馏方法之上。`nuwa-skill` 做的不是"模仿名人说话"，而是尽量从公开材料里提炼一个人的：
...
所以，`clinic` 里的医生不靠口头禅撑风格，而是基于可追溯的认知框架来分析问题。

这一点很关键。如果底层医生只是会"像某个人说话"，多医生会诊最终还是会滑回更花哨的角色扮演。
...
所以 `clinic` 的重点从来不是"模仿人格"，而是搭出一种更接近真实会诊的推理秩序：
...
- 仓库里不只有 skill 文件
- 还有 trigger、routing、doctor selection、end-to-end、multi-turn 的评测样本和 baseline
...
- 普通 persona prompt 通常只有风格差异
- `clinic` 追求的是框架差异、判断差异，以及不同解释路径之间的清晰分离

**不是普通 multi-agent**

- 很多 multi-agent 系统只是让多个 agent 平铺输出
- `clinic` 把主持人层单独拿出来，专门处理分诊、冲突组织和总结失真问题

**不是单独使用 `nuwa-skill`**

- `nuwa-skill` 的强项是把一个人物蒸馏成可调用的 perspective skill
- `clinic` 的新增价值是把这些 perspective skill 编排成一个完整工作流，解决几个核心问题：
  - 什么时候该进诊所
  - 该由谁来回答
  - 分歧怎么保留
  - 多轮追问后怎么不跑偏
...
这个项目从一开始就不只是一份 skill 文件。

仓库里的评测拆成了几层：

- `trigger`：该不该进入诊所
- `routing`：该分到哪种模式
- `doctor_selection`：选医生有没有张力和覆盖
- `end_to_end`：整场会诊是否保留分歧且有用
- `multi_turn`：多轮追问后会不会漂、会不会压扁上下文
...
但它不是"模型偷偷记住你"的黑箱记忆，而是放在本地 `memory/` 目录里的**可审计病历系统**。
...
更重要的是：`clinic` 不会默认偷偷写入记忆。每次会诊后，如果有值得沉淀的内容，它只会提出一个"可写入病历"建议；只有用户确认后，才会更新本地 `memory/` 文件。

这让 `clinic` 不只是一次性会诊，而是可以逐步成为一个长期私人诊所：它会积累经验，但这些经验始终可查看、可修改、可删除。
...
如果你关心的不只是"怎么写一个 skill"，而是"怎么把一个 skill 做成能迭代的系统"，这个仓库就是为这种问题准备的。

### 鸣谢

这个项目要明确感谢 [nuwa-skill](https://github.com/alchaincyf/nuwa-skill)。

`clinic` 里很多医生侧 skill 的基础思路，直接受益于 `nuwa-skill` 对"蒸馏一个人的思维方式"这件事的系统化方法。没有这条思路，`clinic` 很容易退化成人设拼贴，或者退化成名人语气模仿。

更准确地说：

- `nuwa-skill` 提供了医生侧的蒸馏方法论
- `clinic` 在此基础上继续做了主持人层、分诊规则、会诊流程、分歧保留和评测闭环

所以这不是"顺手借了几个角色来用"，而是明确站在前人的方法上继续往前做。
...
- an evaluation stack designed for continuous iteration
...
The real starting point was simpler: I am the kind of person who often has several competing ways of thinking about the same thing. In practice, those lines of thought usually have nowhere to go — not because nobody can understand them, but because very few people are both willing to stay with them and sharp enough to respond in a way that actually moves the problem forward.

So the thoughts just keep circling.

At some point, a more specific idea appeared: what if I could build something where the people I genuinely respect could look at my ideas with me?

The first version of this project was actually called **"Arena."** If ideas need to be tested, put them in the ring.

But pretty quickly I realized that not every problem wants a fight. Sometimes you do not want to win or lose. Sometimes you are simply stuck and need someone to help you see what the problem even is.
...
What `nuwa-skill` extracts is not style. It distills:
...
If the underlying doctors only "sound like" famous people, a multi-doctor consultation is still just theatrical roleplay. `clinic` depends on the stronger claim: the doctors should differ in reasoning structure, not only in tone.

### Design Philosophy

This project rests on a few direct beliefs:
...
The point of `clinic` is not persona simulation. It tries to build a more disciplined reasoning order:

- the host triages, selects doctors, organizes conflict, and writes the diagnosis
- the doctors contribute from genuinely different frameworks
...
`clinic` is not "ask several AIs to impersonate famous thinkers and let them speak in turns."
...
- full `clinic` output
- simple scores and noted limitations
...
- a normal assistant tends to converge quickly toward an answer
...
- `clinic` adds the missing system layer:
  ...
  - how the whole workflow is evaluated and tightened over time
...
- `trigger`: should the system enter Clinic at all
- `routing`: which mode should be used
- `doctor_selection`: whether the roster has enough coverage and tension
- `end_to_end`: whether the consultation is actually useful without flattening disagreement
- `multi_turn`: whether follow-up rounds drift, reset, or compress context poorly

It also draws on ideas from external benchmarks:

- `AgencyBench`
- `PersonaEval`
- `PersonaChat`

The goal is not to prove the skill is impressive. The goal is to make it criticizable, measurable, and improvable.

### Auditable Memory

`clinic` is adding a lightweight memory layer.

This is not hidden model memory. It is an auditable local record system stored under `memory/`.
...
- routing clues between the user's phrasing and actual intent
- concise high-signal notes from each consultation
...
Most importantly, `clinic` does not silently write memory. After a consultation, it may propose a memory update; only after user confirmation does it write to local `memory/` files.

This makes `clinic` more than a one-off consultation. It can gradually become a long-term personal clinic — accumulating knowledge about you while keeping that knowledge visible, editable, and deletable.
...
If you care not only about how to write a skill, but how to turn one into a system that improves over time, this repository is for that problem.

### Acknowledgment

This project should explicitly acknowledge [nuwa-skill](https://github.com/alchaincyf/nuwa-skill).

Much of the foundation for the doctor-side skills in `clinic` benefits directly from `nuwa-skill`'s method for distilling how a person thinks from public material. Without that foundation, `clinic` would be much more likely to collapse into persona stacking or voice imitation.

A more accurate description:

- `nuwa-skill` provides the doctor-distillation methodology
- `clinic` builds the orchestration layer on top: triage, consultation structure, disagreement preservation, and evaluation

So this repository is not merely "using a few characters." It is taking a prior method seriously and extending it into a full consultation system.
