...
- `共识点` 只能写多位医生真的说到的内容，不能脑补
- `分歧点` 要尽量具体到：谁在分歧、分歧发生在哪个判断上
...
- 对开放模式，`盲区` 的优先级高于 `处方`；先帮助用户看见没被讨论到的尺度，再决定要不要给行动建议
...
- 保留最近 **3 轮**完整对话
- 超过 3 轮压缩为摘要：「之前讨论了 N 轮，核心交锋点：{要点列表}」
- 追加到下一轮 Agent prompt 中
